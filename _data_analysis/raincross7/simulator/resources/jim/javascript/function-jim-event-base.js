    ...
    "executeActions": function(actions, caseCallback) {
      if(jimUtil.exists(actions)) {
    	...
    	for (a=0, aLen=array.length;a<aLen; a+=1) {
		  ...
		  if((!self.event.backupState || (self.event.backupState && self.event.type=="click")) && jimUtil.exists($firer.data("jimUndoStack"))) {
		  	/* event in-between mouseenter and mouseleave of a mouseover */
		  	/* enter also if a toggle has been triggered */
		  	undoStack = $firer.data("jimUndoStack");
		    for(l = $firer.data("jimUndoStack").length-1; l >= 0; l -= 1) {
		      lastUndo = $firer.data("jimUndoStack")[l];
		      if(lastUndo.action) {
                for(var p = 0; p < lastUndo.action.length; ++p) {  
			      lastUndoAction = lastUndo.action[p];
			      ...
			      if(lastUndo.type === "mouseenter") {
			        if (current.action === "jimChangeStyle") {
			    	    ...
			    	    for(var j = 0; j < current.parameter.length; ++j) {	
                          ...
                          if(currentParameters.target && undoParameters.target) {    
                            for(var k = 0; k < currentParameters.target.length; ++k) {  
                              var currentTarget = currentParameters.target[k];
			    		      if(undoParameters.target[0] == currentTarget) {
			        ...
			        } else if(current.action === "jimSetValue" && lastUndoAction.action === "jimSetValue") {
                      var undoParameters = lastUndoAction.parameter;
					  var currentParameters = current.parameter;    
                      if(currentParameters.target && undoParameters.target) {
			    	  	for(var k = 0; k < currentParameters.target.length; ++k) {  
                          var currentTarget = currentParameters.target[k];
			    		  if($(undoParameters.target[0])[0] == $(currentTarget)[0]) {
                            for (var i = 0; i < undoParameters.target.length; i++) {
                              if (("#" + undoParameters.target[i].id) == currentTarget) { 
                                undoParameters.target.splice(i, 1);
                                break;
                              }
                            }
                          }
              ...
              if($firer.closest(".datalist, .datagrid").length) {
                if($firer.closest(".headerrow").length) {
                  ...
                  if($target.hasClass("datarow")) {
                     ...
                     */
                	 var $parentsAndSelf = $firer.closest(".datarow,.gridcell").andSelf();
        ...
        case "jimChangeStyle":
          ...
          for(s=0, len=action.parameter.length; s<len; s+=1) {
			...
			for(t=0, tLen=style.target.length; t<tLen; t+=1) {
              ...
              else if (property == "opacity" && $target.jimGetType() == itemType.path) {
                undoStyle.attributes[property] = $target.find("path, rect, circle, ellipse, line, polyline, polygon, use").not("defs *").attr("opacity");
              }
                        	else
                        	  undoStyle.attributes[property] = $target.css(property);
                          }
